...
The function `volume_prism(base_area, height)` calculates the volume of a prism with the provided dimensions. The user provides the `base_area`, which is the area of the base of the prism, and the `height` which is the height dimension.
...
The function `volume_right_circular_cone(base_radius, height)` calculates the volume of a circular cone with the provided dimensions. The user provides the `base_radius`, which is the radius of the base of the cone, and the `height` which is the height dimension.

The inputs for this parameter must be numeric and greater than 0, or the function will throw an error.

```{r}
#volume_right_circular_cone(20, 5)
```
